from .models import DbTag, DbEnhancedNote, enhanced_note_tag_table
...
def json_all(db):
    result = get_base_query(db).group_by(DbTag.id).order_by(func.count('enhanced_note_tag.enhanced_note_id').desc()).all()
